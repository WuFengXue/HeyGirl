...
typedef struct DexMapItem {
    u2 type;
    /* type code (see kDexType* above) */
    u2 unused;
    u4 size;
    /* count of items of the indicated type */
...
typedef struct DexFieldId {
    u2 classIdx;
    /* index into typeIds list for defining class */
    u2 typeIdx;
...
typedef struct DexMethodId {
    ...
    /* index into typeIds list for defining class */
    u2 protoIdx;
    /* index into protoIds for method prototype */
...
typedef struct DexProtoId {
    ...
    u4 returnTypeIdx;
    /* index into typeIds list for return type */
...
static jobject dump_ClassObject_DvmDex_MemMap(JNIEnv *env, jclass obj, jclass arg0, jint version) {
    ...
    if (ver >= 14) {
        ...
        descDvmDex(dvm_dex);
        jobject byte_buffer = (*env)->NewDirectByteBuffer(env, dvm_dex->memMap.addr,
...
static jobject dump_DexFile_mCookie_DexOrJar_memMap(JNIEnv *env, jclass obj, jlong cookie,
                                                    jint version) {
    DexOrJar *pDexOrJar = (DexOrJar *) cookie;
    LOGV("the pDexOrJar mCookie=%d", pDexOrJar);
    descDexOrJar(pDexOrJar);
    ...
    if (ver >= 14) {
        ...
        if (pDexOrJar->isDex) {
            if (pDexOrJar->pDexMemory == NULL) {
                ...
                jobject byte_buffer = (*env)->NewDirectByteBuffer(env, pDexOrJar->pDexMemory,
                                                                  dexHeader->fileSize);
                if (byte_buffer == NULL) {
                    return NULL;
                }
...
static jobject getHeaderItemPtr(JNIEnv *env, jclass obj, jlong mCookie, jint version) {
    ...
    jclass dexFileHeadersPointer_class = (*env)->FindClass(env,
    ...
    jfieldID stringIdField = (*env)->GetFieldID(env, dexFileHeadersPointer_class, "pStringIds",
                                                "I");
    ...
    jfieldID methodIdField = (*env)->GetFieldID(env, dexFileHeadersPointer_class, "pMethodIds",
                                                "I");
    ...
    jfieldID classdefsField = (*env)->GetFieldID(env, dexFileHeadersPointer_class, "pClassDefs",
                                                 "I");
    ...
    jfieldID classCountField = (*env)->GetFieldID(env, dexFileHeadersPointer_class, "classCount",
                                                  "I");
    (*env)->SetIntField(env, dexFileItemInfo_obj, classCountField,
...
static jobject getInlineOperation(JNIEnv *env, jclass obj) {
    ...
    dvmGetInlineOpsTableLengthPtr dvmGetInlineOpsTableLength = dlsym(libdvm,
    ...
    jmethodID stringbuilder_append = (*env)->GetMethodID(env, stringBuilder_class, "append",
                                                         "(Ljava/lang/String;)Ljava/lang/StringBuilder;");
    jmethodID tostring_method = (*env)->GetMethodID(env, stringBuilder_class, "toString",
                                                    "()Ljava/lang/String;");
...
static jobject getSyslinkSnapshot(JNIEnv *env, jclass obj) {
    ...
    jmethodID put_method = (*env)->GetMethodID(env, hashmap_class, "put",
    ...
    while (fgets(line, sizeof(line), m)) {
        ...
        for (i = 0; i < dinfo.nrels; i++) {
            ...
            if (ELF32_R_SYM(rel.r_info)) {
                ...
                jobject long_obj = (*env)->NewObject(env, long_class, init_long_Method,
                                                     fuction_point);
